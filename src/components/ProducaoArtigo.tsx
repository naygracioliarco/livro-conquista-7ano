...
function ProducaoArtigo() {
  const [texto, setTexto] = useState('');

  // Carrega o texto salvo do localStorage ao montar o componente
  useEffect(() => {
    const textoSalvo = localStorage.getItem('producaoArtigo');
    if (textoSalvo) {
      setTexto(textoSalvo);
    }
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem('producaoArtigo', texto);
  }, [texto]);
  ...
  return (
    <div className="my-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        >
          Produção final – Artigo de divulgação científica
        </h2>
      </div>
